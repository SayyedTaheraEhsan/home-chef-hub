...
// GET all recipes
router.get('/', (req, res) => {
    const query = 'SELECT id, title, description, created_by, created_at FROM recipes';

    db.query(query, (err, results) => {
        if (err) return res.status(500).json({ error: 'Error fetching recipes' });
...
router.get('/:id', (req, res) => {
    ...
    db.query(query, [recipeId], (err, results) => {
        if (err) return res.status(500).json({ error: 'Error fetching recipe' });
